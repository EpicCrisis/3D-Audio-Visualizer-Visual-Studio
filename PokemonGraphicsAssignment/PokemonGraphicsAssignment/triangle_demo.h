	...
	void draw(const Matrix& viewMatrix)
	{
		...
		*/
		...
		for (int j = 0; j < 2; j++)
		{
			...
			// Lower leg. ---//
			Matrix leftLowerLegTranslate1 = Matrix::makeTranslationMatrix(Vector(0.0f, -2.75f, -0.55f));
			...
			// First feet position. ---//
			Matrix feetTranslate1 = Matrix::makeTranslationMatrix(Vector(-0.5f, 0.0f, -1.0f));
